...
class DbLocateCountries{
	...
	*/
	...
	public function Import_From_CSV($filename,$table_name = "location_geo_countries", $progress_callback = null) {
		if (!file_exists($filename)) {
			throw new DBIP_Exception("file {$filename} does not exists");
		}
		$f = @fopen($filename, "r");
		...
		//	\Log::info("insert into" .' '.$table_name.' '. "(country, city,latitude, longitude) VALUES ". "(".'\''.$import['country'].'\''.",".'\''.$import['city'].'\''.",".'\''.$import['latitude'].'\''.",".'\''.$import['longitude'].'\''.")");
			$q = $this->db->prepare("insert into" .' '.$table_name.' '. "(country,latitude, longitude) VALUES ". "(".'\''.$import['country'].'\''.",".'\''.$import['latitude'].'\''.",".'\''.$import['longitude'].'\''.")");
			$this->Import_Row($q);
			}
			$nrecs++;
		}
