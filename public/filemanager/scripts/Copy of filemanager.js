...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		if($.urlParam('config') != 0) {
			...
			var url = '/filemanager/scripts/filemanager.config.js';
...
$(function(){
	...
	// Loading theme
	loadCSS('/filemanager/themes/' + config.options.theme + '/styles/filemanager.css');
	$.ajax({
	    url:'/filemanager/themes/' + config.options.theme + '/styles/ie.css',
	    async: false,
	    success: function(data)
	    {
	        $('head').append(data);
